...
The main dsh renderer receives the desktop protocol marker and an operation that opens the dedicated plugin window. The separate plugin window receives structured list, install, remove, update, and update-check operations; neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments. Each sandboxed preload is a self-contained bundle.

The desktop profile includes native Crew and its Session-owned right-sidebar tab. Windows window close offers hiding, full exit, or cancellation. Full exit confirms active work, prepares task owners, flushes Session storage, and terminates the backend process tree before quitting. Plugin changes and update installation refuse to restart an active Host. Usage statistics use the trusted private Fetch channel and the official Session read handles through a version-pinned compatibility patch. The Agent picker and Settings roster preserve bundle-provided presets, including selectable Orchestration mode; Standard mode remains the default.
...
The Windows x64 command requires Windows x64. `desktop:dist:linux` retains Linux x64 target selection for a matching host. This downstream repository accepts the current Crew phase on native Windows; Linux runtime qualification is deferred, and retained macOS helpers carry no downstream release commitment. See the [maintenance scope](../../.agents/notes/implemented/process/2026-09-05-windows-linux-maintenance-scope.md).
