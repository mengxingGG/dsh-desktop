...
describe('desktop upload plan', () => {
  it('validates Linux AppImage metadata and rejects modified bytes', async () => {
    ...
    await writeFile(join(paths.artifactsRoot, 'deepseek-harness-1.2.3-linux-x64.AppImage'), 'tampered')
    await expect(createDesktopUploadPlan('linux-x64', paths)).rejects.toThrow(/does not match update metadata/u)
  })
