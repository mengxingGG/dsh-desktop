...
describe('desktop host process', () => {
  it.runIf(process.platform === 'win32')('stops unregistered descendants before releasing the Windows root', async () => {
...
descendant.once('spawn', () => process.send({ type: 'ready', protocolVersion: 3, dshVersion: String(descendant.pid) }))
function onRequestFrame() {}
`)
    const host = new DesktopHostProcess(process.execPath, project)
    try {
      const ready = await host.start()
      const pid = Number(ready.dshVersion)
      ...
      await expect(host.start()).rejects.toThrow('unavailable')
    } finally {
      await host.stop()
    }
  })
