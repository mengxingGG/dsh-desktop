...
describe('desktop project transactions', () => {
  ...
  it('reconciles dsh to the packaged release without removing desktop plugins', async () => {
    ...
    const usagePatch = 'patches/@ychris12138__dsh-usage-stats@0.2.9.patch'
    expect(readFileSync(join(paths.profile, 'pnpm-workspace.yaml'), 'utf8')).toContain(usagePatch)
    expect(readFileSync(join(paths.profile, usagePatch), 'utf8')).toContain('trustedFetch')
