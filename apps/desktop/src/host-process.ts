...
import { desktopControl, type DesktopControl } from './control.ts'
import { parseDesktopResponse } from '@deepseek-ai/dsh-web-app/desktop-protocol'
import {
...
export class DesktopHostProcess {
  private control: DesktopControl | undefined
   ...
   * @param environment - Host environment, including the application's isolated DSH home.
   * @param allowLinkedProfile - Permit workspace links in an unpackaged development profile.
   */
  constructor(
    ...
    private readonly allowLinkedProfile = inspectPort !== undefined,
  ) {}

  /** Start the child once and resolve only after its complete composition is active. */
  async start(): Promise<DesktopHostReady> {
    if (this.stopped) throw new Error('dsh desktop host is unavailable')
    ...
    this.child = child
    this.control = desktopControl(child, 30_000)
    ...
    child.on('message', (message: unknown) => {
      try {
        if (parseDesktopResponse(message) !== undefined) return
      } catch (error) {
        this.fail(errorOf(error, 'dsh desktop host sent an invalid control response'))
        child.kill('SIGTERM')
        return
      }
  ...
  /** Inspect active tasks through the Host's private IPC channel. */
  async activity(): Promise<boolean> {
    await this.readyPromise
    if (this.control === undefined) throw new Error('dsh desktop host is unavailable')
    return this.control.activity()
  }

  /** Stop and persist tasks, returning false when active work needs confirmation. */
  async shutdown(confirmed: boolean): Promise<boolean> {
    await this.readyPromise
    if (this.control === undefined) throw new Error('dsh desktop host is unavailable')
    if (!await this.control.shutdown(confirmed)) return false
    await this.stop()
    return true
  }

  /** Terminate the backend process tree and await exit; use shutdown() to flush durable state first. */
  async stop(): Promise<void> {
    ...
    if (child.pid !== undefined && child.exitCode === null && child.signalCode === null) {
      if (process.platform === 'win32') {
        // Keep the root alive until taskkill has enumerated its descendants.
        await new Promise<void>((resolve, reject) => {
          execFile('taskkill', ['/pid', String(child.pid), '/T', '/F'], { windowsHide: true, timeout: 5_000 }, (error) => {
            if (error !== null && child.exitCode === null && child.signalCode === null) reject(new Error('Could not terminate the desktop Host process tree', { cause: error }))
            else resolve()
          })
        })
      } else {
    ...
    } else if (child.connected) this.send({ type: 'shutdown' })
