...
const DESKTOP_PROFILE_BUNDLES = [
  ...
  '@deepseek-ai/dsh-crew-profile', '@deepseek-ai/dsh-crew-web-profile',
] as const
...
const USAGE_PATCH = '@ychris12138__dsh-usage-stats@0.2.9.patch'

function copyCompatibilityPatch(projectDir: string): void {
  mkdirSync(join(projectDir, 'patches'), { recursive: true, mode: 0o700 })
  copyFileSync(new URL(`../patches/${USAGE_PATCH}`, import.meta.url), join(projectDir, 'patches', USAGE_PATCH))
}
...
function workspaceFile(overrides: Readonly<Record<string, string>> = {}): string {
  ...
  return `packages:\n  - .\n\n${overrideSection}${WORKSPACE_SETTINGS}patchedDependencies:\n  '@ychris12138/dsh-usage-stats@0.2.9': patches/${USAGE_PATCH}\nallowBuilds:\n  node-pty: true\n  koffi: true\n  fs-ext: true\n  ${JSON.stringify(coreBuildKey)}: true\n  '@google/genai': false\n  protobufjs: false\n  node-addon-require-builtin: false\n`
}
...
function copyMetadata(source: string, target: string): void {
  ...
  cpSync(join(source, 'patches'), join(target, 'patches'), {
    recursive: true,
    force: false,
    errorOnExist: true,
  })
...
export function createSeedMetadata(seedDir: string, release: DesktopRelease): void {
  ...
  copyCompatibilityPatch(seedDir)
...
export function createDevelopmentProjectMetadata(projectDir: string, release: DesktopRelease): void {
  ...
  copyCompatibilityPatch(projectDir)
  const manifest = {
    name: PROJECT_NAME,
    private: true,
    version: '0.0.0',
