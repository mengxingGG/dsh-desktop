...
Windows x64 命令要求 Windows x64。`desktop:dist:linux` 为匹配宿主保留 Linux x64 目标选择。本下游仓库在原生 Windows 上验收当前 Crew 阶段；Linux 运行时验证暂缓，保留的 macOS 辅助命令不构成下游发布承诺。参阅[维护范围](../../.agents/notes/implemented/process/2026-09-05-windows-linux-maintenance-scope.zh.md)。
