---
description: "Run named agents with durable messages and shared tasks, including the native Crew workflow."
...
`dsh-agent-team` turns a coding Session into a team of named helpers with durable messages and shared tasks. Teammates recover queued messages after interruptions. The service supplies the native Crew workflow; generic self-organizing model tools remain an optional experimental Consumer. Durable Session storage is required.
...
Host workflow consumers may reserve the exact child Session id before materialization, attach a typed source to the initial prompt, and supply supported model-route, persona, inherited-tool filter, and depth values. Agent Teams forwards these values to the continuable-child owner, which persists the child composition for cold resume. The ordinary Team model tool omits these host-only controls.
...
Every ordinary runtime root is the implicit Lead of a Team whose `TeamId` equals its `SessionId`; there is no creation event, and durable state begins with the first member, message, or task record. `spawnTeammate()` first appends and flushes a `provisioning` member record with either the caller-reserved child id or a generated id, then asks the configured provider to create that exact child; a provider failure appends a durable `failed` member. A fresh child starts with no Lead history; a fork child captures the Lead's completed-turn prefix once. The initial prompt retains caller-owned provenance, while route, persona, tool-filter, and depth values enter the continuable descriptor. Recovery reconciles an unterminated provisioning record against the child's independently persisted Session: a matching direct-parent and continuable descriptor plus a recorded initial user message produces `active`, and anything else produces `failed`. If recovery wins a same-process race, the creator accepts the terminal state or reports `TEAM_PROVISIONING_CONFLICT` and drains the child. Names are reserved by the first provisioning record and never reused.
...
- [tool-agent-team package](../../experimental/tool-agent-team/README.md) — the tools that let the model create, message, and coordinate teammates.
...
- **Pre-stable API** — service changes require matching updates to Crew and experimental Consumers.
...
The [Crew delivery decision](../../../.agents/notes/implemented/feature/2026-09-06-native-crew-default-delivery.md) owns release inclusion and the remaining experimental Consumers.
