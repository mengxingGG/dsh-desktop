---
description: "运行具名智能体、持久消息和共享任务，支持原生 Crew 工作流。"
...
`dsh-agent-team` 把编码 Session 组织为具名助手团队，提供持久消息和共享任务。成员在中断后恢复排队消息。该服务支持原生 Crew 工作流；通用自组织模型工具仍是可选实验 Consumer。运行需要持久 Session 存储。
...
宿主工作流 Consumer 可以在物化前预留精确子 Session id，为初始提示附加类型化 source，并提供受支持的模型路由、persona、继承工具筛选与深度值。Agent Teams 会把这些值转发给可续接子智能体所有者，后者持久化子组合供冷恢复使用。普通 Team 模型工具不暴露这些仅宿主控制项。
...
每个普通运行时 root 都是一个隐式 Team 的 Lead，其 `TeamId` 等于 `SessionId`；不存在创建事件，持久状态从第一条成员、消息或任务记录开始。`spawnTeammate()` 先追加并 flush 一条 `provisioning` 成员记录，其中使用调用方预留的 child id 或生成的 id，再要求配置的 provider 创建这个精确 child；provider 失败会追加一条持久的 `failed` 成员。fresh child 不携带 Lead 历史；fork child 只捕获一次 Lead 的已完成 turn 前缀。初始提示保留调用方拥有的 provenance，路由、persona、工具筛选与深度值则进入 continuable descriptor。恢复把未终结的 provisioning 记录对照 child 独立持久化的 Session 进行对账：直接 parent 与 continuable descriptor 匹配、且初始用户消息已记录则产生 `active`，其他任何情况都产生 `failed`。如果恢复在同进程竞争中先完成，creator 会接受终态，或报告 `TEAM_PROVISIONING_CONFLICT` 并 drain 该 child。名字由第一条 provisioning 记录保留，且永不复用。
...
- [tool-agent-team 包](../../experimental/tool-agent-team/README.zh.md)——让模型创建、消息与协调 teammate 的工具。
...
- **预稳定 API**——服务变化需要同步更新 Crew 与实验 Consumer。
...
[Crew 交付决策](../../../.agents/notes/implemented/feature/2026-09-06-native-crew-default-delivery.zh.md) 拥有发布包含规则与剩余实验 Consumer 的说明。
