...
describe('Team identity and provisioning', () => {
  ...
  it('forwards a reserved identity and cold-resumable child composition', async () => {
    const { ctx, lead } = await setup(['hang'])
    const reserved = SessionId('00000000-0000-4000-8000-000000000456')

    const started = await ctx.agentTeams.spawnTeammate(lead, {
      childId: reserved,
      name: 'composed-worker',
      description: 'composed worker responsibility',
      prompt: content('implement the assigned module'),
      context: 'fresh',
      provider: 'spawn',
      agentOptions: { provider: 'mock', model: 'mock' },
      persona: 'Crew developer persona',
      toolFilter: { allow: [] },
      maxDepth: 1,
      signal: SIGNAL,
    })

    expect(started.member.id).toBe(reserved)
    const worker = await waitRunning(ctx, reserved)
    expect(worker.options).toMatchObject({ provider: 'mock', model: 'mock' })
    expect(worker.session.header.delegationDepth).toBe(1)
    expect(worker.session.snapshotEvents().find(event => event.type === 'subagent/descriptor')?.data)
      .toMatchObject({
        agentProvider: 'mock',
        agentModel: 'mock',
        persona: 'Crew developer persona',
    ...
    expect(ctx.agentTeams.interrupt(lead, 'composed-worker')).toEqual({ previousStatus: 'running' })
    await waitNoAgent(ctx, reserved)
  })
