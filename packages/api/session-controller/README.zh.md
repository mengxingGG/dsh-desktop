...
模型目录与选择同时支持已注册的 Agent 执行器和 LLM 适配器。提交前会根据执行器模型校验推理选项与图像支持。
...
模型路由优先使用待应用的显式 Session 选择，其次是已记录的请求头，最后才是该精确 Agent 的[已注册默认值](../../core/agent-default-model/README.zh.md)。提示变量和请求路由捕获同一份选择；改变岗位默认值不会替换已有会话选择。
