...
Model catalogs and selection include registered Agent executors alongside LLM adapters. Executor models validate reasoning choices and image support before submission.
...
Model routing prefers a pending explicit Session selection, then the recorded request header, then the exact Agent's [registered default](../../core/agent-default-model/README.md). Prompt variables and request routing capture the same selection; changing a role default does not replace an existing conversation choice.
