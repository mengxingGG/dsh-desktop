...
在[应用退出准备](../../../.agents/notes/implemented/feature/2026-09-05-windows-tray-durable-exit.zh.md)中，工厂除回合外也报告启动及维护工作，关闭新 Agent 准入，并等待全部自有拆除工作结束后才返回聚合错误。重复准备请求保留同一个结果，包括最终 Session 写句柄关闭失败。
