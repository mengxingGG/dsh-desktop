...
External steps invoke `agents/execute` with loop-owned transcript and tool callbacks. Host admission middleware preserves the callbacks' permission and persistence checks.
...
For [application exit preparation](../../../.agents/notes/implemented/feature/2026-09-05-windows-tray-durable-exit.md), the factory reports active startup and maintenance as well as turns, closes new Agent admission, and joins every owned teardown before returning an aggregate failure. Repeated preparation requests retain the same outcome, including a failed final Session writer close.
