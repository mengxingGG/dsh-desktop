...
The `agents/execute` waterfall wraps admitted external execution and delegates through `next(input)`. Middleware can account for requests and wrap `executeTools`, preserving cancellation, transcript ownership, and tool settlement.
