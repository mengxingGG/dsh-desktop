...
1. **Roster** — the composed profile's plugin rows (standard YAML parse under Include's dialect, `!!js` intact), plus the rows of every config tree the CLI declares in its `package.json` `dsh.configTrees` (agent presets), materialized as a Node-style dependency closure. Roster resolution uses the selected profile's dependency links; transitive dependencies resolve from their installed importer. Conflicting installed copies remain under the importer's own `node_modules`. External peer edges never bind the worker; workspace peers stay on the chain.
...
`withComposedProfile` keeps a temporary default profile and the normal launcher's dependency links alive until its consumer settles. Pass its composition fields to `packVfsImage` inside that callback; paths from the callback are invalid after it returns. Both success and failure remove the private profile home. The pack result counts installations by their path below the image's `node_modules`, including nested copies.
