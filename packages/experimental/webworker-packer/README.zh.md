...
1. **Roster**——合成 profile 的插件行（标准 YAML 解析、Include 方言、`!!js` 原样保留），加上 CLI 在 `package.json` `dsh.configTrees` 里声明的每棵配置树（agent presets）的行，按 Node 式依赖闭包物化。Roster 通过所选 profile 的依赖链接解析；传递依赖从其已安装的导入方解析。相互冲突的安装副本保留在导入方自己的 `node_modules` 下。外部包的 peer 边不追，workspace peer 保留在链上。
...
3. **可达性 sweep**——用运行时加载器自己的解析，从每个 roster 包、全部 workspace 导出面及 worker 装配种子（`IMAGE_ENTRY_SEEDS`）出发，pack 时把每个可达模块降低到包装契约。Transform 会报告具名静态 import、re-export 与动态 import、经 `require` 发起的调用，以及通过 `node:module` 或 `module` 具名导入（含导入别名）在模块作用域直接发起的 `createRequire(import.meta.url)('pkg')` 调用。页面资产（`./client` 导出背后的 `lib/client.js`）原样直发；自家代码的不可解析请求打包即失败，第三方的容忍到 require 时 fail loud。
...
`withComposedProfile` 会保留临时默认 profile 及正常启动器建立的依赖链接，直到其消费者完成。应在该回调内把合成字段传给 `packVfsImage`；回调结束后，返回的路径失效。成功和失败都会删除私有 profile 主目录。打包结果按镜像 `node_modules` 下的安装路径统计包，包括嵌套副本。
