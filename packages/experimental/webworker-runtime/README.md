...
- **`node:dns/promises`, `node:https`, `node:vm`, `node:net`, `node:sqlite`, `node:worker_threads` have structural stubs**: unavailable operations report their refusal on the console and throw. HTTPS imports support plugin loading, but `request` and `get` never substitute browser fetch or bypass the caller's DNS and TLS checks. Rows needing native DNS, sockets, a real process, or realm isolation cannot run here.
