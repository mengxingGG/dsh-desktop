...
- **`node:dns/promises`、`node:https`、`node:vm`、`node:net`、`node:sqlite`、`node:worker_threads` 提供结构化 stub**：不可用操作会在 console 报告拒绝并抛出。HTTPS 导入支持插件加载，但 `request` 与 `get` 不会替换为浏览器 fetch，也不会绕过调用方的 DNS 和 TLS 检查。需要原生 DNS、socket、真进程或真 realm 隔离的行在此无法运行。
