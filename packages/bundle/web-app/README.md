...
The default roster mounts the GitHub marketplace as ordinary Host and Client plugins: [`plugin-marketplace-github`](../../host/plugin-marketplace-github/README.md) owns discovery and profile installation, while [`ui-settings-plugin-marketplace`](../../client/ui-settings-plugin-marketplace/README.md) owns the Settings cards and confirmation flow. A deployment can disable or replace either Cordis config entry without changing the Web host or desktop shell.

The roster also mounts the version-pinned MIT community plugin [`@ychris12138/dsh-usage-stats`](https://github.com/Ychris12138/dsh-usage-stats) as the `usage-stats` Cordis config entry. Its dual Host/Client package owns the loopback account and aggregated-usage endpoints, local fold cache, sidebar action, provider cards, token totals, cache-hit rate, and daily heatmap. The dependency remains external source with its upstream license and tests; an overlay can disable or replace the row.
...
- **Usage analytics is version-pinned external code** — configured accounts refresh every five minutes even when the panel is closed; disable the `usage-stats` row to remove its local cache, background work, and provider requests from a deployment.
