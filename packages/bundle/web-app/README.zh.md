...
默认名录把 GitHub 插件市场作为普通 Host 与 Client 插件挂载：[`plugin-marketplace-github`](../../host/plugin-marketplace-github/README.zh.md) 负责发现与 profile 安装，[`ui-settings-plugin-marketplace`](../../client/ui-settings-plugin-marketplace/README.zh.md) 负责设置卡片与确认流程。部署方可以停用或替换任一 Cordis 配置项，而无需修改 Web 宿主或桌面壳。

该名录还把固定版本的 MIT 社区插件 [`@ychris12138/dsh-usage-stats`](https://github.com/Ychris12138/dsh-usage-stats) 作为 `usage-stats` Cordis 配置项挂载。它的 Host/Client 双面包负责回环账户与聚合用量端点、本地折叠缓存、侧边栏操作、供应商卡片、token 总量、缓存命中率与每日热力图。该依赖仍使用带上游许可证和测试的外部源码；overlay 可以停用或替换该配置项。
...
- **用量统计是固定版本的外部代码**：已配置账户即使在面板关闭时也会每 5 分钟刷新；停用 `usage-stats` 配置项可从部署中移除其本地缓存、后台工作与供应商请求。
