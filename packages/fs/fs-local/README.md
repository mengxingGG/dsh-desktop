...
Path-level `lstat` reports the host's observed hard-link count and distinguishes symbolic links without following the final path component. This metadata lets stricter consumers reject file aliases; the local backend does not reject hard links on its own.
