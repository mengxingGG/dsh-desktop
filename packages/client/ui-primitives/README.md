...
`AnimatedIcon` uses Morphicons for state changes such as copy-to-check and panel expansion. Icons are decorative; containing controls own localized labels. Motion honors `prefers-reduced-motion` and never loops continuously.
...
`WebBlock` reserves room for three-digit citation ordinals in its scrolling source list.
