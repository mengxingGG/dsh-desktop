...
describe('RepositoryCleaner', () => {
  it('derives live build outputs from project references and removes safe stale package residue', async () => {
    ...
    write(join(root, 'DeepSeek-Harness.exe'))
    write(join(root, 'DeepSeek-Harness.AppImage'))
    write(join(root, 'DeepSeek-Harness.app/Contents/MacOS/DeepSeek Harness'))
    write(join(root, 'apps/desktop/dist/backend/node'))
    ...
    expect(existsSync(join(root, 'DeepSeek-Harness.exe'))).toBe(false)
    expect(existsSync(join(root, 'DeepSeek-Harness.AppImage'))).toBe(false)
    expect(existsSync(join(root, 'DeepSeek-Harness.app'))).toBe(false)
