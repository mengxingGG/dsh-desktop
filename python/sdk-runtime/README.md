...
Production executables are named `deepseek-harness-sdk-runtime-<platform>-<arch>` under the module's `runtime/` directory; Windows uses the `.exe` suffix. Linux and macOS wheels include a target-native `-rg` sidecar, Windows includes `-rg.exe`, and macOS also includes `-spawn-helper` for `node-pty`. Maintained release targets are Linux x64, Linux arm64, and Windows x64. macOS definitions and sidecar handling remain for contributor builds without new release wheels; see the [maintenance scope](../../.agents/notes/implemented/process/2026-09-05-windows-linux-maintenance-scope.md). The wheel tag and payload must match exactly; no Windows arm64 wheel is published.
