...
生产可执行程序位于模块的 `runtime/` 目录，命名为 `deepseek-harness-sdk-runtime-<platform>-<arch>`；Windows 使用 `.exe` 后缀。Linux 与 macOS wheel 包含目标平台原生的 `-rg` 伴随程序，Windows 包含 `-rg.exe`，macOS 还包含 `node-pty` 使用的 `-spawn-helper`。维护中的发布目标是 Linux x64、Linux arm64 与 Windows x64。macOS 定义和伴随程序处理保留用于贡献者构建，不发布新的 wheel；见[维护范围](../../.agents/notes/implemented/process/2026-09-05-windows-linux-maintenance-scope.zh.md)。Wheel tag 必须与载荷严格匹配；不发布 Windows arm64 wheel。
