...
- The [shipped Web composition test](../../../../apps/web/tests/shipped-composition.e2e.ts) boots the Web profile on native Windows, checks that its catalog includes `pwsh` and excludes `bash`, and exercises the preset producer through the host registry's `job_list` and `job_output` tools. The expected shell is selected by the host platform, not inferred from the observed catalog, so a missing or incorrect default fails the test.
