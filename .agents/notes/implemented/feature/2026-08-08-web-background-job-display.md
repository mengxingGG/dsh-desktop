...
The keyless [Web e2e scenario](../../../../apps/web/tests/background-job-list.e2e.ts) invokes the shipped native shell (`pwsh` on Windows, `bash` on POSIX), observes the new header count without reloading, and opens its job list. It kills the owned job through `ctx.jobs`, waits for a `killed` registry snapshot, and checks the open list's producer detail. [Platform-specific expected outputs](../../../../apps/web/tests/expected/background-job-list/) live beside the test because the borrowed Session supplies only browser setup, not the background job; Windows retains its native cancellation detail rather than rewriting it as a POSIX signal.
