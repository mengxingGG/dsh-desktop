...
The runtime package's `platforms.json` is the source of truth for native wheel tags and executable names. The repository release builder and the isolated Hatch build hook validate and load that file independently. The manifest retains macOS definitions for local builds, but GitHub Actions and GitLab CI select only Windows/Linux carriers and do not invoke macOS deployment checks.
