...
Team 领域按照 [Crew 交付决策](../feature/2026-09-06-native-crew-default-delivery.zh.md)在 `packages/subagent/agent-team` 发布。`packages/experimental/tool-agent-team`、`packages/experimental/agent-team-profile`、`packages/experimental/client-ui-agent-team` 和 `packages/experimental/agent-team-web-profile` 仍是私有消费方。[实验性包规则](../../../../packages/experimental/AGENTS.md)负责其发布排除与晋升要求。
...
通用的调用方预留 continuable child 身份和精确 direct-child drain 仍属于稳定 Subagent 服务。它们负责 Subagent 身份与 Activation 生命周期，不 import 或命名 Agent Teams；Team 服务消费这些能力，不反转该依赖方向。
...
私有 Team 消费方可以使用完整仓库依赖图与质量检查，而不进入正式 tarball。CLI 实验安装显式的私有 profile 层；通用 profile launcher 可以接受该层，而不会让它的 plugin 依赖进入 dsh 发布闭包。已发布 Team 领域仍供 Crew 和这些私有消费方使用。
