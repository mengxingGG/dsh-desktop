...
**镜像保留按导入方区分的依赖解析。** 默认组合通过真实 CLI 在私有 Harness 主目录下生成。随后，`withComposedProfile` 建立正常启动器的所选 profile 依赖链接，并保留到打包回调完成，包含异步消费者；回调结束即删除私有主目录。外部插件行即使没有被 workspace 静态导入，也属于可达性遍历起点。相互冲突的已安装依赖副本保留在其导入方下方；打包遍历和 Worker `require` 都只在镜像内部沿祖先 `node_modules` 目录查找。按包名扁平化会混合不兼容的 OpenTelemetry 发布版本，导致原本安装有效的 exporter 在构造时失败。`dsh-worker-transform/2` 要求这一解析行为，因此旧 Worker 会拒绝镜像，而不会静默选用另一份依赖。聚焦的 roster-resolution 测试检查实际加载的导出、缺失依赖、导入方本地副本与共享副本身份；构建镜像测试会构造真实 OpenTelemetry exporter 和 logger。
