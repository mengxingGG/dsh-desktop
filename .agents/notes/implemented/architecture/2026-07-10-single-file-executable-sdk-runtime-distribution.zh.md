...
CI 使用 [`.github/workflows/build-exe-for-python-sdk.yml`](../../../../.github/workflows/build-exe-for-python-sdk.yml) 为 Linux x64/arm64 与 Windows x64 构建原生载体；手动运行可选择其中一部分。每个目标通过[安装后 wheel 验证](../testing/2026-08-23-installed-python-wheel-black-box-ci.zh.md)检查干净环境中的产物来源、无密钥场景，以及可信拉取请求中的真实提供方；fork 与 Dependabot head 不获得密钥。Linux 还检查 GLIBC 要求并运行 manylinux 2.28 安装验证。完整集合为一个 SDK wheel 与三个运行时 wheel。[GitHub 发布](../process/2026-08-11-python-publication-workflow.zh.md)与 [GitLab 发布](../../../../.gitlab-ci.yml)只接受这个四文件集合；GitLab 标签必须与仓库版本匹配。裸可执行文件和源码闭包只作为测试中间输入。[Windows x64 运行时决定](2026-08-23-python-sdk-windows-x64-runtime.zh.md)继续明确排除 Windows arm64。
