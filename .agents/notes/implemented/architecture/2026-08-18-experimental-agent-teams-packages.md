...
The Team domain is released at `packages/subagent/agent-team` under the [Crew delivery decision](../feature/2026-09-06-native-crew-default-delivery.md). `packages/experimental/tool-agent-team`, `packages/experimental/agent-team-profile`, `packages/experimental/client-ui-agent-team`, and `packages/experimental/agent-team-web-profile` remain private consumers. The [experimental package rules](../../../../packages/experimental/AGENTS.md) own their release exclusion and promotion requirements.
...
The generic caller-reserved continuable child identity and selective direct-child drain remain in the stable Subagent service. They own Subagent identity and Activation lifecycle without importing or naming Agent Teams; the Team service consumes them without reversing that dependency.
...
Private Team consumers can use the full repository graph and quality checks without entering official tarballs. The CLI experiment installs an explicit private profile layer; the generic profile launcher accepts that layer without making its plugin dependencies part of the dsh release closure. The released Team domain remains available to Crew and these private consumers.
